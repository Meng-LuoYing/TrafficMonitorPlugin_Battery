...
namespace
{
    ...
    constexpr int ID_DEVICE_SYNC_EDIT = 1002;
    constexpr int ID_BATTERY_REFRESH_EDIT = 1003;
    constexpr int ID_APPLY_BUTTON = 1004;
    ...
    LRESULT CALLBACK PortDialogProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_CREATE:
        {
            ...
            state->hNetworkGroup = CreateWindowW(L"BUTTON", L"网络设置",
                WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 16, 16, 488, 82, hWnd, nullptr, nullptr, nullptr);
            state->hTimingGroup = CreateWindowW(L"BUTTON", L"刷新设置",
                WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 16, 108, 488, 124, hWnd, nullptr, nullptr, nullptr);
            ...
            state->hDeviceSyncLabel = CreateWindowW(L"STATIC", L"设备变动检测间隔（秒，1-3600）：",
                WS_CHILD | WS_VISIBLE, 32, 138, 250, 20, hWnd, nullptr, nullptr, nullptr);
            state->hBatteryRefreshLabel = CreateWindowW(L"STATIC", L"电量刷新间隔（秒，1-3600）：",
                WS_CHILD | WS_VISIBLE, 32, 182, 250, 20, hWnd, nullptr, nullptr, nullptr);
            ...
            state->hDeviceSyncEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", syncText,
                WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 290, 134, 194, 24,
            ...
            state->hBatteryRefreshEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", refreshText,
                WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 290, 178, 194, 24,
        ...
        case WM_COMMAND:
        {
            ...
            if ((cmd == IDOK || cmd == ID_APPLY_BUTTON) && state && state->hPortEdit && state->hDeviceSyncEdit && state->hBatteryRefreshEdit)
            {
                wchar_t portText[32] = {};
                wchar_t syncText[32] = {};
                ...
                {
                    MessageBoxW(hWnd, L"设备变动检测间隔必须是 1 到 3600 之间的整数秒。", L"蓝牙电量", MB_OK | MB_ICONWARNING);
                ...
                {
                    MessageBoxW(hWnd, L"电量刷新间隔必须是 1 到 3600 之间的整数秒。", L"蓝牙电量", MB_OK | MB_ICONWARNING);
    ...
    bool ShowOptionsDialogWindow(HWND parent, int currentPort, int currentDeviceSyncSec, int currentBatteryRefreshSec, int& resultPort, int& resultDeviceSyncSec, int& resultBatteryRefreshSec)
    {
...
ITMPlugin::OptionReturn BatteryPlugin::ShowOptionsDialog(void* hParent)
{
    ...
    bool accepted = ShowOptionsDialogWindow(reinterpret_cast<HWND>(hParent), currentPort, currentDeviceSyncSec, currentBatteryRefreshSec, newPort, newDeviceSyncSec, newBatteryRefreshSec);
    if (!accepted)
        return OR_OPTION_UNCHANGED;

    bool changed = (newPort != currentPort) || (newDeviceSyncSec != currentDeviceSyncSec) || (newBatteryRefreshSec != currentBatteryRefreshSec);
    if (!changed)
        return OR_OPTION_UNCHANGED;
